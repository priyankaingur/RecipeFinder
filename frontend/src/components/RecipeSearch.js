...
function RecipeSearch() {
    const {...ingredients} = useField('text', '')
    const [recipes, setRecipes] = useState([]);
    const [mostSearchedRecipes,setMostSearchedRecipes]=useState([]);

    useEffect(() => {
        Recipe.getMostViewed()
            .then(response => setMostSearchedRecipes(response))
            .catch(error => console.error('There was an error fetching the recipes!', error));    }, []);
    const searchRecipes = () => {
        Recipe.getByIngredients(ingredients.value)
    ...
    return (
        <div
            className="bg-second-surface bg-opacity-20 mx-4 border-2 rounded-2xl p-4 m-2 border-primary">
            <h2 className="text-2xl font-bold mb-4">Search Recipes by
                Ingredients</h2>
            ...
            {recipes?.length > 0 ? <ListRecipes recipes={recipes}/> :
                <div className="flex flex-col">
                    ...
                    <ListRecipes recipes={mostSearchedRecipes}/>
                </div>
            }
